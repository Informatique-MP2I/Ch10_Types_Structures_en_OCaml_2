...
let () = 
  ...
  Matrix.set sys_mat 0 0 4.;
  Matrix.set sys_mat 0 1 1.;
  Matrix.set sys_mat 1 0 2.;
  Matrix.set sys_mat 1 1 (-3.); 
  let cons_mat = Matrix.create 2 1 0.0 in
  Matrix.set cons_mat 0 0 6.;
  Matrix.set cons_mat 1 0 (-4.);
  let sol_mat = resolve_2_2_Gauss sys_mat cons_mat in
  Matrix.print sys_mat;
  Matrix.print cons_mat;
  Matrix.print sol_mat
*)  

let () = 
  let sys_mat = Matrix.create 2 2 0.0 in
  Matrix.set sys_mat 0 0 7.5;
  Matrix.set sys_mat 0 1 (-2.);
  Matrix.set sys_mat 1 0 3.;
  Matrix.set sys_mat 1 1 (4.5);
  let cons_mat = Matrix.create 2 1 0.0 in
  Matrix.set cons_mat 0 0 3.;
  Matrix.set cons_mat 1 0 33.;
  let sol_mat = resolve_2_2_Gauss sys_mat cons_mat in
  Matrix.print sys_mat;
  Matrix.print cons_mat;
  Matrix.print sol_mat
